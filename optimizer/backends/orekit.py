...
class OrekitOptimizationBackend:
    """Orekit-backed single-candidate optimization adapter."""

    backend_id = "orekit"
    module_id = "orekit_single_candidate"
    ...
    def solve(self, problem: dict[str, Any], out_dir: Path, run: bool = False) -> dict[str, Any]:
        ...
        if run and status != "failed":
            run_result = run_generated_python(simulation_out / "generated_mission.py")
        ...
        return {
            ...
            "limitations": [
                "Orekit optimization backend currently performs single-candidate two-body evaluation; iterative search is not implemented yet."
            ],
