...
from mission_compiler.backends.capabilities import check_capability
from mission_compiler.hashing import hash_file
from mission_compiler.io import read_json, write_json, write_text
from mission_compiler.validation.validate_frames import gmat_frame
from mission_compiler.ir.sequence import normalize_mission_sequence, iter_steps
from mission_compiler.dependencies.spice import write_spice_requests, build_spice_requests, build_spice_kernel_sets
from mission_compiler.reference_frames import normalize_reference_frame_declarations, frame_to_gmat_coordinate_system, AXIS_SUFFIX
from mission_compiler.visualization import write_visualization_manifest
from mission_compiler.time_formats import format_epoch_for_backend
...
def output_reports(spec: dict, out_dir: str | Path | None = None) -> list[dict]:
    ...
    for out in spec.get("outputs", []):
        ...
        if out.get("type") == "ground_track":
            ...
            report_path = (base / relative_path).resolve() if base is not None else Path(relative_path)
            reports.append({
                ...
                "body": body,
                "path": relative_path,
                "script_path": _script_report_path(report_path, relative_path),
                "source_filename": Path(relative_path).name,
                "desired_path": str(report_path).replace("\\", "/"),
...
def derived_segment_reports(spec: dict, reports: list[dict], out_dir: str | Path | None = None) -> list[dict]:
    ...
    for report in reports:
        ...
        for segment in segments:
            ...
            desired = (base / rel).resolve() if base is not None else Path(rel)
            derived.append({
                "name": _safe_name(f"SEG_{report['name']}_{segment['step_id']}"),
                "kind": "derived_segment",
                "path": rel,
                "desired_path": str(desired).replace("\\", "/"),
...
class GmatCompiler:
    ...
    def __init__(self) -> None:
        base = Path(files("mission_compiler.backends.gmat"))
    ...
    def compile(self, spec: dict, out_dir: str | Path) -> dict:
        ...
        if stm_outputs:
            ...
            write_json(contract_path, {
                ...
                "consumer": "mission_targeting.solve_stm_target_state_correction",
            })
            artifacts.append({
