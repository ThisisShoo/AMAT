...
EXPECTED_OUTPUTS = [
    str(ROOT / "outputs/_Ephemeris_MeoDemoSat_EarthMJ2000Eq.csv"),
    str(ROOT / "outputs/_Ephemeris_MeoDemoSat_EarthFixed.csv"),
    str(ROOT / "outputs/_GroundTrack_MeoDemoSat_Earth.csv"),
]
REPORT_COPIES = [
    {"source_filename": "_Ephemeris_MeoDemoSat_EarthMJ2000Eq.csv", "desired_path": r"D:/Documents and stuff/Random stuff/Pet Projects/Agnetic GMAT/AMAT/generated/MEO_demo/simulation/outputs/_Ephemeris_MeoDemoSat_EarthMJ2000Eq.csv"},
    {"source_filename": "_Ephemeris_MeoDemoSat_EarthFixed.csv", "desired_path": r"D:/Documents and stuff/Random stuff/Pet Projects/Agnetic GMAT/AMAT/generated/MEO_demo/simulation/outputs/_Ephemeris_MeoDemoSat_EarthFixed.csv"},
    {"source_filename": "_GroundTrack_MeoDemoSat_Earth.csv", "desired_path": r"D:/Documents and stuff/Random stuff/Pet Projects/Agnetic GMAT/AMAT/generated/MEO_demo/simulation/outputs/_GroundTrack_MeoDemoSat_Earth.csv"},
]
CHECKPOINT_REPORTS = [
]
...
def build_objects() -> dict[str, Any]:
    ...
    MeoDemoSat = gp.Spacecraft("MeoDemoSat")
    MeoDemoSat.SetField("DateFormat", "UTCGregorian")
    MeoDemoSat.SetField("Epoch", "01 Jan 2026 00:00:00.000")
    MeoDemoSat.SetField("CoordinateSystem", "EarthMJ2000Eq")
    MeoDemoSat.SetField("DisplayStateType", "Keplerian")
    MeoDemoSat.SetField("SMA", 6878.1363)
    MeoDemoSat.SetField("ECC", 0.0)
    MeoDemoSat.SetField("INC", 60.0)
    MeoDemoSat.SetField("RAAN", 0.0)
    MeoDemoSat.SetField("AOP", 0.0)
    MeoDemoSat.SetField("TA", 0.0)
    MeoDemoSat.SetField("DryMass", 1000.0)
    MeoDemoSat.SetField("Cd", 2.2)
    MeoDemoSat.SetField("Cr", 1.8)
    MeoDemoSat.SetField("DragArea", 8.0)
    MeoDemoSat.SetField("SRPArea", 12.0)
    objects["meo_demo_sat"] = MeoDemoSat
    ...
    EarthProp = gp.PropSetup(
        "EarthProp",
        fm=objects["earth_8x8"],
        gator=EarthProp_integrator,
        initial_step_size=30.0,
        accuracy=1e-09,
        min_step=0.1,
        max_step=300.0,
    )
...
def build_mission_sequence(objects: dict[str, Any]) -> list[Any]:
    mcs: list[Any] = []
    # Phase: finite_raise - Finite-burn orbit raise
    ...
    mcs.append(
        gp.Propagate(
            "Propagate_propagate_12_days_MeoDemoSat",
            objects["meo_demo_sat"],
            objects["earth_prop"],
            ("MeoDemoSat.ElapsedSecs", 1036800.0),
        )
    )
    return mcs
...
def run_direct_python() -> dict[str, Any]:
    ...
    before = {
        "meo_demo_sat": _get_state(objects["meo_demo_sat"]),
    }
    run_code = gp.RunMission(mcs)
    after: dict[str, Any] = {}
    try:
        runtime_sc = _gmat_module().GetRuntimeObject("MeoDemoSat")
    except Exception:
        runtime_sc = objects["meo_demo_sat"]
    after["meo_demo_sat"] = _get_state(runtime_sc)
    visualization_export = _auto_export_visualization_artifacts()
    return {
        "mission_id": MISSION_ID,
