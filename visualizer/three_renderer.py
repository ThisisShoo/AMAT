...
def _trace_payload(trace: EphemerisTrace, mission_dir: Path, max_points: int = 6000) -> dict[str, Any] | None:
    df = trace.dataframe
    ...
    return {
        ...
        "path": _display_path(trace.path, mission_dir),
...
def _ground_track_payload(track: Any, mission_dir: Path, max_points: int = 12000) -> dict[str, Any] | None:
    df = track.dataframe
    ...
    return {
        ...
        "path": _display_path(track.path, mission_dir),
        "time": [float(v) for v in t],
...
def _scene_payload(scene: MissionScene, paths: MissionPaths) -> dict[str, Any]:
    ...
    checkpoints = sorted(
        ...
        key=lambda item: (
            float("inf") if item.get("elapsed") is None else float(item["elapsed"]),
...
def render_three_html(scene: MissionScene, paths: MissionPaths, output: str | Path | None = None) -> Path:
    ...
    payload = json.dumps(_scene_payload(scene, paths), ensure_ascii=False, allow_nan=False)
